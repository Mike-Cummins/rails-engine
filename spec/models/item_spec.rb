...
RSpec.describe Item, type: :model do
  ...
  before :each do
    ...
    @min_max_items = [@item_1, @item_3]
    @max_items = [@item_1, @item_2, @item_3]
    
  end
  it 'Finds all items by min and max price search' do

    # expect(Item.find_all_by_price(10, 20)).to eq(@min_max_items.sort)
  end

  it 'Finds all items by max price only search' do

    # expect(Item.find_all_by_price(nil, 20)).to eq(@max_items.sort)
  end
end